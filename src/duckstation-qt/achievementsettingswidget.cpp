  ...
  : QWidget(parent), m_host_interface(host_interface)
{
  ...
  if (m_host_interface->GetBoolSettingValue("Cheevos", "Enabled", false))
    host_interface->OnAchievementsLoaded();
}
...
{
  const bool logged_in = Cheevos::IsLoggedIn();

  if (logged_in)
  {
    // not strictly thread safe...
    m_ui.loginStatus->setText(tr("Username: %1").arg(QString::fromStdString(Cheevos::GetUsername())));
...
void AchievementSettingsWidget::onLoginLogoutPressed()
{
  ...
  {
    m_host_interface->executeOnEmulationThread([]() {
      Cheevos::Logout();
    }, true);
    updateLoginState();
    return;
